...
def load_images_rgb(image_paths: List[Path], rotate=None) -> List[np.ndarray]:
    ...
    for image_path in image_paths:
        ...
        if rotate is not None:
            print(f"Rotating image {image_path} by {rotate} degrees")
...
def infer_vggt_and_reconstruct(
    ...
    depth_tensor = predictions["depth"] # torch.Size([1, 9, 392, 518, 1])
    depth_conf = predictions["depth_conf"]
    # print(f"Depth tensor shape: {depth_tensor.shape}, Depth conf shape: {depth_conf.shape}")

    # # plot depth map and confidence map for all frames as grid
    # import matplotlib.pyplot as plt
    ...
    #     ax_depth = axes[i, 0] if rows > 1 else axes[0]
    #     ax_conf = axes[i, 1] if rows > 1 else axes[1]
    #     ax_depth.imshow(depth_tensor[0, i, :, :, 0].cpu().numpy(), cmap='plasma')
    #     ax_depth.set_title(f'Depth Map Frame {i}')
    #     ax_depth.axis('off')
    #     ax_conf.imshow(depth_conf[0, i, :, :].cpu().numpy(), cmap='viridis', vmin=0, vmax=1)
    #     ax_conf.set_title(f'Depth Confidence Frame {i}')
    #     ax_conf.axis('off')
    # plt.tight_layout()
    # plt.show()
